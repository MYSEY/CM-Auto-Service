        ...
        <div id="panel-2" class="panel">
            <div class="panel-hdr bg-white border-bottom-0 h6 font-weight-bold text-uppercase" style="min-height: 65px; padding: 0 1.5rem;">
                <h2 class="text-primary d-flex align-items-center">
                    ...
                    Edit <span class="fw-300 text-muted ml-1">Slider</span>
                </h2>
            </div>
                ...
                <div class="panel-content p-4">
                    <form method="POST" action="{{ url('admins/slide',$data->id) }}" enctype="multipart/form-data" novalidate>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control rounded-lg" name="title" id="title" placeholder="Enter slider title" value="{{ $data->title }}">
                            <small class="text-danger font-weight-bold">{!! $errors->first('title') !!}</small>
                        </div>

                        <div class="form-group">
                            <label for="image_slider" class="font-weight-bold text-dark">Change Slider Image</label>
                            <div class="figma-dropzone p-3 text-center border rounded-lg" style="background: #f8fafc; border: 2px dashed #cbd5e1 !important; cursor: pointer;">
                                <i class="fal fa-cloud-upload fa-2x text-primary mb-2 d-block"></i>
                                <span class="font-weight-bold text-dark d-block mb-1">Click to select new image</span>
                                <input type="file" name="image_slider" class="form-control-file d-block mx-auto" id="image_slider" accept="image/*">
                            </div>
                        ...
                        <div id="photo-preview" class="mb-3">
                            <label class="font-weight-bold text-dark d-block">Current Slider Image:</label>
                            @if ($data->image_slider)
                                <div id="main-photo-container" class="d-inline-block border p-2 rounded bg-light shadow-sm">
                                    <img id="main-product-photo" src="{{ asset('images/sliders/' . $data->image_slider) }}" class="rounded" style="object-fit: cover; width: 220px; height: 110px;" alt="Slider image">
                                </div>
                            @else
                                <p id="main-photo-placeholder" class="text-muted italic">No main photo currently set.</p>
                            @endif
                        </div>
                        ...
                        <div class="form-group mb-0 text-right mt-4 pt-3 border-top">
                            ...
                            <button type="submit" id="submit-btn" class="btn btn-primary rounded-pill px-4 font-weight-bold shadow-sm">
                                <i class="fal fa-check-circle mr-1"></i> Update Slider
                            </button>
                        </div>
                    </form>
                </div>
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        ...
                        if (typeof toastr !== 'undefined') {
                            toastr.success(res.message || 'Update Slider successfully.');
                        }
                        if (res.redirect) {
                            setTimeout(function() {
